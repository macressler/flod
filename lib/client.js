...
Client.prototype.startBench = function (callback) {
    ...
    var precalcWrapper = function (cb) {
        ...
        if (!self.hasOwnProperty('_precalc') || !self._precalc.hasOwnProperty('concurrents')) {
...
Client.prototype.bench = function (callback) {
    ...
    // precalcWrapper(function (err, result) {
...
Client.prototype._bench = function (index, concurrents, max, precalc, callback) {
    ...
    // if ((index*concurrents) > (max - concurrents)) return callback(null);

    var self = this;
    var handler = self._request(self.options['-m'], self.url.href, index*concurrents);
    
    // var requestsToMake = concurrents;
    // if ( (max - (index*concurrents)) < concurrents ) {
    //     requestsToMake = max - (index*concurrents);
    // }
