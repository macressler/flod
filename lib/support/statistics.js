...
Statistics.prototype.calcStats = function (stats) {
  ...
  result['runtime'] = parseInt(stats.ended) - parseInt(stats.started);
...
Statistics.prototype._tableDataFns = {
  ' ': {
    ...
    get: function(data, files, index){ return [files[index].info.server, "@", files[index].info.version].join("") + " (.." + files[index].ended.toString().slice(-4) + ".json)"}
  },
  "total/conc": {
    delta: false,
    get: function(data){ return [data.totalReq, data.concReq].join('/'); }
  },
  "runtime": {
    get: function(data){ return data.runtime; },
  },
  "avg rps": {
    get: function(data){ return data.rps.mean; },
  },
  "avg lat": {
    get: function(data){ return data.lat.mean; },
  },
  "avg mem": {
    get: function(data){ return data.mem.mean; },
  },
  "avg load": {
    get: function(data){ return data.load.mean; },
  },
}
  ...
  for(var f in files){
    ...
    if (+f > 0) {
      ...
      for(var i = 2; i <= rows; i++){
        ...
        if (this._tableDataFns[table[i][0]].delta !== false) {
